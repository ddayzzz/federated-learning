...
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    数据的统计信息
    :param y_train: 标签
    :param net_dataidx_map:  client->[训练的数据的index]
    :return: 各个 local 网络中类别出现的数量
    """
    net_cls_counts = {}

    for net_i, dataidx in net_dataidx_map.items():
        unq, unq_cnt = np.unique(y_train[dataidx], return_counts=True)  # [类], [类出现的次数]
        tmp = {unq[i]: unq_cnt[i] for i in range(len(unq))}  # 类->出现的次数
        net_cls_counts[net_i] = tmp  # 某个客户端的类->数量统计
...
def partition_data(dataset, datadir, partition, n_nets, alpha, args):
    ...
    :param dataset: 数据集名称
    :param datadir: 数据的目录
    :param partition: 划分的类型: homo, hetero-dir 等
    :param n_nets: 局部网络的数量
    :param alpha: ???
    :param args: 传入的程序参数
    :return: y_train 标签, net_dataidx_map: 局部网络index->[所拥有的数据的 index], traindata_cls_counts: {局部网络index->{类->出现的数量}}
    """
    if dataset == 'mnist':
    ...
    # 确定切分的类型
    if partition == "homo":
        idxs = np.random.permutation(n_train)  # [0, n_train] 的随机排列
        batch_idxs = np.array_split(idxs, n_nets)  # n_nets 是FL中的客户端数量, 每个客户随机分配 n_train / n_nets 的训练数据
...
def init_models(net_configs, n_nets, args):
    ...
    for cnn_i in range(n_nets):
        if args.model == "lenet":
            ...
            cnn = UNet(n_channels=1, n_classes=1, bilinear=True)
        cnns[cnn_i] = cnn
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None):
    ...
    if dataset in ('mnist', 'cifar10'):
        ...
        training_dir = os.path.sep.join(('D:', "Projects","pytorch_brats2018_fl","data","brats2018","training"))
        ...
        train_dl = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
        test_dl = torch.utils.data.DataLoader(val, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True)
