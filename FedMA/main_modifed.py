...
from matching_performance import compute_model_averaging_accuracy, compute_pdm_cnn_accuracy, compute_pdm_vgg_accuracy, \
    compute_full_cnn_accuracy
...
def trans_next_conv_layer_backward(layer_weight, next_layer_shape):
    ...
    next_layer_shape[1], next_layer_shape[0], next_layer_shape[2], next_layer_shape[3])
    reshaped = layer_weight.reshape(reconstructed_next_layer_shape).transpose(1, 0, 2, 3).reshape(next_layer_shape[0],
                                                                                                  -1)
    return reshaped
...
def local_train(nets, args, net_dataidx_map, device):
    """
    生成客户端的数据加载器 [train,valid]
    :param nets:
    :param args:
    ...
    for net_id, net in nets.items():
        if args.retrain:
            ...
            trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl_global, args.epochs, args.lr, args,
                                          device=device)
...
def local_retrain_dummy(local_datasets, weights, args, mode="bottom-up", freezing_index=0, ori_assignments=None,
                        device="cpu"):
    """
    FOR FPNM
    """
...
def BBP_MAP(nets_list, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device):
    # starting the neural matching
    models = nets_list
    cls_freqs = traindata_cls_counts  # 在客户端中 class->count
    n_classes = args_net_config[-1]
    it = 5

    assignments_list = []
    # 得到网络参数的权重, conv 变成向量形式, 且在 CPU 中
...
def BBP_MAP_loaded(nets_list, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device):
    # starting the neural matching
    models = nets_list
    n_classes = args_net_config[-1]
    # 得到网络参数的权重, conv 变成向量形式, 且在 CPU 中
    ...
    with open('./matching_weights_cache/assignments_list', 'rb') as fi:
        assignments_list = pickle.load(fi)


    with open('./matching_weights_cache/matched_layerwise_weights', 'rb') as fi:
        batch_weights = pickle.load(fi)
    matched_weights = []
    num_layers = len(batch_weights[0])
    last_layer_weights_collector = []
        ...
        if param_idx < 1:  # we need to handle the 1st conv layer specificly since the color channels are aligned
            ...
            if "bias" in key_name:  # the last bias layer is already aligned so we won't need to process it
                ...
                _temp_res_weight = __reconstruct_weights(weight=_matched_weight, assignment=prev_assignment,
                                                         layer_ori_shape=_layer_ori_shape,
                                                         matched_num_filters=_matched_num_filters,
...
def fedma_comm(batch_weights, model_meta_data, layer_type, net_dataidx_map,
    ...
    for cr in range(comm_round):
        ...
        for worker_index in range(args.n_nets):
            ...
            recons_local_net = reconstruct_local_net(batch_weights[worker_index], args,
        ...
        # BBP_MAP step
        hungarian_weights, assignments_list = BBP_MAP(retrained_nets, model_meta_data, layer_type, net_dataidx_map,
...
if __name__ == "__main__":
    ...
    if args.partition != "hetero-fbs":
        # 类别, client 的训练数据的 index, client 的训练数据的类出现的数量
        y_train, net_dataidx_map, traindata_cls_counts = partition_data(dataset=args.dataset, partition=args.partition,
                                                                        alpha=args_alpha, args=args, n_nets=args.n_nets,
                                                                        datadir=args_datadir)
    else:
        y_train, net_dataidx_map, traindata_cls_counts, baseline_indices = partition_data(args.dataset, args_datadir,
                                                                                          args_logdir,
                                                                                          args.partition, args.n_nets,
                                                                                          args_alpha, args=args)
    ...
    # nets_list : [train, valid loader]
    nets_list = local_train(nets, args, net_dataidx_map, device=device)
    # 全局模型
    train_dl_global, test_dl_global = get_dataloader(args.dataset, args_datadir, args.batch_size, 32)
    # this is for PFNM
    # todo 用 BBP MAP 的pickle 进行处理; np 和 torch 的随机数种子都是 0
    hungarian_weights, assignments_list = BBP_MAP_loaded(nets_list, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device=device)
    # hungarian_weights, assignments_list = BBP_MAP(nets_list, model_meta_data, layer_type, net_dataidx_map,
    #                                               averaging_weights, args, device=device)
    # with open('./matching_weights_cache/assignments_list', 'wb') as fo:
    ...
    with open('./matching_weights_cache/averaged_weights', 'rb') as fo:
        averaged_weights = pickle.load(fo)
