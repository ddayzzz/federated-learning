...
def BBP_MAP(nets_list, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device):
    ...
    :param nets_list: 列表, 大小为客户端数量;内容为模型的数量
    :param model_meta_data:
    :param layer_type:
    :param net_dataidx_map:
    :param averaging_weights:
    :param args:
    :param device:
    :return:
    """
    ...
    n_layers = int(len(batch_weights[0]) / 2)  # 选取一个网络的cnn的层数的一半
...
def BBP_MAP_loaded(nets_list, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device):
    ...
    return hungarian_weights, assignments_list
...
if __name__ == "__main__":
    ...
    seed = 0  # 设置 np torch 的随即数种子确保实验可以重现
    ...
    if args.partition != "hetero-fbs":
        ...
        y_train, net_dataidx_map, traindata_cls_counts = partition_data(dataset=args.dataset, partition=args.partition, alpha=args_alpha, args=args, n_nets=args.n_nets, datadir=args_datadir)
    else:
    ...
    with open('./matching_weights_cache/averaged_weights', 'wb') as fo:
